...
def radial_freqs( masses, trap_freqs ):
    ...
    full_coulomb_vec = [oofpez * e**2 / 1 * 
        sqrt( 1 / ((xpos[i] - xpos[j])**2 + (ypos[i] - ypos[j])**2 +
            (zpos[i] - zpos[j])**2) ) if numpy.abs(i-j) < 20 else 0
        for j in range(1, nions) for i in range(j)] 

    full_coulomb = sum( full_coulomb_vec )
    rpos = xpos + ypos
    ...
    return (numpy.concatenate( (wsx, wsy,wsz), axis=0 ), 
        numpy.concatenate( (vsx, vsy,vsz), axis=1 ))
...
def draw_plot(storeddata, exptime):
    ...
    for ion, nbar, w, new_w, off in zip( range(2*nions), nbars, ws, new_ws, offsets ):
        ...
        pyplot.annotate( str(int(nbar)), 
            ...
            xytext=(-40 - 5 * (ion==6) + 70 * (ion==2 or ion==3), 30), textcoords='offset points',
...
#            print( "Frequency: {}".format( ws[i] / 2 / numpy.pi / 1e6 ) )
            print( "Vector: {}".format(
                ', '.join( str(vs[c,i]) for c in range(len(masses)) if masses[c] == 138 ) ) )
