...
class IonDetector:
    ...
    def detect( self, data ):
        ...
        darkdata = data[ np.where( data[:, 1:1+nions] - bkgmean < 9000 ) ]
        bridata = data[ np.where( data[:, 1:1+nions] - bkgmean > 9000 ) ]
			
        #plt, bins, _ = pyplot.hist( (darkdata-bkgmean).flatten(), 40, range=(-10000,35000), label="Bright" )
        #briplt, _, __ = pyplot.hist( (bridata-bkgmean).flatten(), bins=bins, label="Dark" )
        #pyplot.xlabel( "Detected Flourescence (counts)" )
        #pyplot.title( "Histogram of Detected Flourescence for State Measurement" )
        #pyplot.legend( [plt, briplt], ["Dark", "Bright"] )
        #pyplot.legend()
        #pyplot.show()
