...
public:
	...
	{
		return (	abs( (double)(x-vec.x) ) < G_EPSILON &&
					abs( (double)(y-vec.y) ) < G_EPSILON );
	}
...
template <typename T>
inline std::ofstream& operator <<( std::ofstream& o, Vector2<T>& v ) {
	o << "(" << v.x << ", " << v.y << ")";
}
...
template <typename T>
inline std::ofstream& operator <<( std::ofstream& o, Vector3<T>& v ) {
	o << "(" << v.x << ", " << v.y << ", " << v.z << ")";
}
...
public:
	...
	//storage for changing this to standard or hessian normal form
	float d;

	Plane(): px(0), py(0), pz(0), nx(0), ny(0), nz(0) {}
	Plane(Vector3<float> p, Vector3<float> n): px(p.x), py(p.y), pz(p.z), nx(n.x), ny(n.y), nz(n.z) {}
	Plane(float _px, float _py, float _pz, float _nx, float _ny, float _nz): px(_px), py(_py), pz(_pz), nx(_nx), ny(_ny), nz(_nz) {}
	~Plane() {}
};
...
public:
	...
	Ray( const Vector3<T>& s, const Vector3<T>& e ): start(s), end(e), direct(e-s)
	{}

	Vector3<T> evaluate( T t ) const {
		return start + direct*t;
	}
};
