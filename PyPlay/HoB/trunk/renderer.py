...
class Camera:
	def __init__( self, position, look, up, clamp_y = False ):
	...
	def _right( self ):
		return numpy.cross( self.look, numpy.array((0., 1., 0.)) )
	...
	def rotate( self, x, y ):
		...
		if self.clamp_y:
			if self.look[1] > 0.9 or self.look[1] < -0.9:
				...
				self.look = numpy.array((scale*self.look[0], \
					y, scale*self.look[2]))
	...
	def update( self, renderer ):
		if self.rotation[0] or self.rotation[1]:
			self.rotate( self.rotation[0], self.rotation[1] )
		if self.translation[0] or self.translation[1]:
