...
void compute_gradient(EquationSystems& es);
...
int main(int argc, char** argv)
{
  ...
  const int max_linear_iterations   = input_file("max_linear_iterations", 25000);
  ...
  if (dim == 1)
    ...
    PolygonHole holes[4] = {
        PolygonHole( Point( -.5, -.5 ), .25, 50 ),
        PolygonHole( Point( .5, -.5 ), .25, 50 ),
        PolygonHole( Point( .5, .5 ), .25, 50 ),
        PolygonHole( Point( -.5, .5 ), .25, 50 ) };

    std::vector< TriangleInterface::Hole* > hole_vec;
    hole_vec.push_back( &holes[0] );
    hole_vec.push_back( &holes[1] );
    hole_vec.push_back( &holes[2] );
    hole_vec.push_back( &holes[3] );

    t.attach_hole_list( &hole_vec );
    t.triangulate();
    ...
    TetGenIO( mesh ).read( "test.1.ele" );

    //mesh.find_neighbors();
    //std::cout << "Found neighbors." << std::endl;

    //TetGenMeshInterface t( mesh );
    //t.triangulate_conformingDelaunayMesh( 2.0, 0.01/*0.00001*/ );
    std::cout << "Loaded mesh." << std::endl;
    ...
    std::cout << mesh.elem(8)->n_nodes() << std::endl;
    for( size_t i = 0; i < mesh.elem(8)->n_nodes(); ++i )
      std::cout << mesh.elem(8)->point(i) << std::endl;
  ...
  LinearImplicitSystem& system =
    equation_systems.add_system<LinearImplicitSystem> ("RF");
  
  // Adds the variable "u" to "Laplace", using 
  // the finite element type and order specified
  // in the config file
  system.add_variable("rf_u", static_cast<Order>(approx_order),
  ...
  LinearImplicitSystem& needle_system =
    equation_systems.add_system<LinearImplicitSystem>( "Needles" );

  needle_system.add_variable("needle_u", static_cast<Order>(approx_order),
			Utility::string_to_enum<FEFamily>(approx_type));
  needle_system.attach_assemble_function (assemble_laplace);

  // Initialize the data structures for the equation system.

  ExplicitSystem& gradient_system =
    equation_systems.add_system<ExplicitSystem> ("Pseudo");
  gradient_system.add_variable("rf_pseudou", CONSTANT, MONOMIAL);
  ...
  for (unsigned int r_step=0; r_step<max_r_steps; r_step++)
    {
      ...
      system.solve();
      needle_system.solve();
      compute_gradient( equation_systems );
      ...
      // Compute the error.
      exact_sol.compute_error("RF", "rf_u");

      // Print out the error values
      std::cout << "L2-Error is: "
                << exact_sol.l2_error("RF", "rf_u")
                << std::endl;
      std::cout << "H1-Error is: "
                << exact_sol.h1_error("RF", "rf_u")
                << std::endl;

      // Print to output file
      out << equation_systems.n_active_dofs() << " "
          << exact_sol.l2_error("RF", "rf_u") << " "
          << exact_sol.h1_error("RF", "rf_u") << std::endl;
                      ...
                      const std::string&) // unk_name, not needed
{
  ...
  if( ( Point(x, y) - Point(-.5, -.5) ).size() < 0.125001 )	return 1.0;
  if( ( Point(x, y) - Point(.5, .5) ).size() < 0.125001 )	return 1.0;
  if( ( Point(x, y) - Point(-.5, .5) ).size() < 0.125001 )	return 0.0;
  if( ( Point(x, y) - Point(.5, -.5) ).size() < 0.125001 )	return 0.0;

  if( z > -.999999 && z < .999999 ) 
	if( (Point(x, y) - Point(0., 0.)).size() < 0.6 ) return 0.;
  return -1.;
}


Number needle_exact_solution(const Point& p,
                      const Parameters&,  // parameters, not needed
                      const std::string&, // sys_name, not needed
                      const std::string&) // unk_name, not needed
{
  ...
  if( z > -.999999 && z < .999999 )
	if( (Point(x, y) - Point(0., 0.)).size() < 0.6 ) return 1.;
  return -1.;
}
              ...
              for (unsigned int qp=0; qp<qface->n_points(); qp++)
                {
                  Number value;
                  if( system_name == "RF" )
                    value = exact_solution (qface_points[qp],
				es.parameters, "null", "void");
                  else
                    value = needle_exact_solution ( qface_points[qp],
				es.parameters, "null", "void" );
...
void compute_gradient(EquationSystems& es) {
  ...
  LinearImplicitSystem& system = es.get_system<LinearImplicitSystem>("RF");
  
  const unsigned int pot = system.variable_number( "rf_u" );
  const DofMap& dof_map = system.get_dof_map();
  FEType fe_type = dof_map.variable_type( pot );
  AutoPtr<FEBase> fe( FEBase::build(dim, fe_type) );
  ...
  ExplicitSystem& gradient_system = es.get_system<ExplicitSystem>("Pseudo");
  //gradient_system.solution->zero();

  const unsigned int pseudo = gradient_system.variable_number( "rf_pseudou" );
  const DofMap& gradient_dof_map = gradient_system.get_dof_map();
  ...
  std::vector<unsigned int> gradient_dof_indices_var;

  for( ; el != end_el; ++el ) {
    const Elem* elem = *el;
    dof_map.dof_indices( elem, dof_indices_var, pot );
    ...
    for( unsigned int qp=0; qp < qrule.n_points(); ++qp ) {
      ...
      for( unsigned int l=0; l < n_var_dofs; ++l )
        //gradient.add_scaled( system.point_gradient( dof_indices_var[l],
        //    q_point[qp], *elem ), JxW[qp] );
        gradient.add_scaled( dphi[l][qp], 
          JxW[qp] * system.current_solution( dof_indices_var[l] ) );

    }

    gradient /= elem->volume();

    gradient_dof_map.dof_indices( elem, gradient_dof_indices_var, pseudo );
    unsigned int dof_index = gradient_dof_indices_var[0];

    if( gradient_system.solution->first_local_index() <= dof_index &&
      dof_index < gradient_system.solution->last_local_index() ) 
      gradient_system.solution->add( dof_index, gradient.size_sq() );
  }

  gradient_system.solution->close();
  gradient_system.update();
}
