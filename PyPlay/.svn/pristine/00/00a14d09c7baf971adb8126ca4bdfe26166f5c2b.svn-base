...
from enthought.traits.api import Bool, Float, Int, Range, HasTraits
from enthought.traits.ui.api import View, Item, Group

class Luca( HasTraits ):
    exposure_time = Float(0.3, label="Exposure Time")
    EMCCD_gain = Range(1, 200, label="EMCCD Gain")
    bin_size = Range(1, 4, label="Bin Size") 
    acquiring = Bool(False, label="Acquiring")
    
    def _EMCCD_gain_default( self ):
        return 200
    def _bin_size_default( self ):
        return 2
    
    view = View( Group(    Item('acquiring', style='readonly'),
                            Item('exposure_time'),
                            Item('EMCCD_gain'),
                            Item('bin_size'),
...
if __name__ == "__main__":
    ...
    import numpy, numpy.ctypeslib
    
    print luca.image
    print luca.image[100]
    
    plt.imshow( luca.image )
    plt.show()
