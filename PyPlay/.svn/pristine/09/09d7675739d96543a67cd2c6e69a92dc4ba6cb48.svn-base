...
class FeatureFactory:
	...
	"""
        ...
        propType = "none"
        props = ["aboard", "about", "above", "across", "afore", "after",
          "against", "along", "alongside", "amid", "amidst", "among",
	  "amongst", "around", "aside", "at", "before", "behind", "below",
	  "beneath", "beside", "besides", "between", "by", "down", "during",
	  "for", "from", "in", "inside", "into", "near", "of", "off", "on",
	  "onto", "outside", "over", "since", "than", "through", "throughout",
	  "till", "to", "toward", "towards", "under", "underneath", "until",
	  "unto", "up", "upon", "via", "with", "within", "without" ]
	if position != 0 and words[ position-1 ] in props:
            propType = words[ position-1 ]
        elif position > 1 and words[ position-2 ] in props:
            propType = words[ position-2 ]

        #personprop = ["by", "of", "with"]
        #if position != 0 and words[ position-1 ] in personprop and \
        #  position > 1 and words[ position-2 ] in personprop:
        #    propType = "personprop"

        #locationprop = ["at", "near", "in"]
        #if position != 0 and words[ position-1 ] in locationprop and \
        #  position > 1 and words[ position-2 ] in locationprop:
        #    propType = "locationprop"

        features.append("case=" + case)
        features.append("propType=" + propType)
        features.append("case=" + case + ", prevLabel=" + previousLabel)
        features.append("case=" + case + ", propType=" + propType)

        if position < len(words)-1 and words[ position+1 ] == "'s":
            features.append("possessive")

	if len(currentWord) > 2 and currentWord[2:] == "O'":
            features.append("contractedname")
        elif len(currentWord) > 3 and currentWord[3:] == "al-":
            features.append("contractedname")
