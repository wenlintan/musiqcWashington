...
class genetic:
    ...
    def generation( self ):
        ...
        for x in range( size ):
            ...
            # select index based on its scores
            # higher scores have higher probability of being selected
            ind1 = [i for i in range( slen ) if sum( scores[0:i+1] ) > r1][0]
            ind2 = [i for i in range( slen ) if sum( scores[0:i+1] ) > r2][0]

            newfac = self.breed( self.factions[ind1], self.factions[ind2] )
            self.factions.append( newfac )
    ...
    ##################################################################
    #utility functions performing one step of process
    #mutation of randomization for the most part
    
    def randomize( self ):
        for fac in self.factions:
                
            self.randomize_ideal_values( fac, fac.ideal_values )
    ...
    ##################################################################
    # functions generating complicated objects
    ...
    def randomize_ideal_values( self, fac, current ):
        for k in current.keys():
            current[k] = self.rand_int( -100, 100 )
    

    ##################################################################
    #helper functions for commonly necessary random operations
